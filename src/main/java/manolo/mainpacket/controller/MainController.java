 ...
 * This class manages the interaction between the application's views and models, handling user inputs and application logic.
 *
...
public class MainController {
    ...
    public void submitRegister() {
        ...
        if (areFieldsFilled()) {
            if (Utils.isValidEmail(registerView.getTextFields().get(2).getText())) {
                if (Utils.isStrongPassword(new String(registerView.getPasswordFields().getFirst().getPassword()))) {
                    if (Utils.isValidDNI(registerView.getTextFields().get(0).getText())) {
                        if (!userExists) {
